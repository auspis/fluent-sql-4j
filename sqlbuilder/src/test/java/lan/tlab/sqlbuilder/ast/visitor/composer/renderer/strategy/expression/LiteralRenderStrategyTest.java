...
class LiteralRenderStrategyTest {
    ...
    @Test
    void string() {
        String sql = strategy.render(Literal.of("string"), sqlRenderer);
        assertThat(sql).isEqualTo("'string'");
    }

    @Test
    void number() {
        String sql = strategy.render(Literal.of(23), sqlRenderer);
        assertThat(sql).isEqualTo("23");

        sql = strategy.render(Literal.of(23.023), sqlRenderer);
        assertThat(sql).isEqualTo("23.023");
    }

    @Test
    void booleanValue() {
        String sql = strategy.render(Literal.of(true), sqlRenderer);
        assertThat(sql).isEqualTo("true");

        sql = strategy.render(Literal.of(false), sqlRenderer);
        assertThat(sql).isEqualTo("false");
    }

    @Test
    void nullValue() {
        String sql = strategy.render(Literal.ofNull(), sqlRenderer);
