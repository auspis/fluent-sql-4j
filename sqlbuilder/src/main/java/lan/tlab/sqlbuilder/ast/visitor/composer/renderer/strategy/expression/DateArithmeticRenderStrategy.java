...
public interface DateArithmeticRenderStrategy extends ExpressionRenderStrategy {
    ...
    public String render(DateArithmetic functionCall, SqlRenderer sqlRenderer);

    public static DateArithmeticRenderStrategy standardSql2008() {
        return (functionCall, sqlRenderer) -> String.format(
                "%s %s %s",
                functionCall.getDateExpression().accept(sqlRenderer),
                functionCall.isAdd() ? "+" : "-",
                functionCall.getInterval().accept(sqlRenderer));
    }
    ...
    public static DateArithmeticRenderStrategy sqlServer() {
        return (functionCall, sqlRenderer) -> String.format(
                ...
                functionCall.isAdd() ? "" : "-",
                functionCall.getInterval().getValue().accept(sqlRenderer),
                functionCall.getDateExpression().accept(sqlRenderer));
    }
