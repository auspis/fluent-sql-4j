...
class MySQLDialectPluginTest {
    ...
    @Test
    void shouldHaveDialectVersionConstant() {
        assertThat(MySQLDialectPlugin.DIALECT_VERSION).isNotNull();
        // Version uses semantic versioning with caret notation for MySQL 8.x compatibility
        assertThat(MySQLDialectPlugin.DIALECT_VERSION).isEqualTo("^8.0.0");
    }
    ...
    @Test
    void shouldUseMySQLNameConstant() {
        assertThat(MySQLDialectPlugin.DIALECT_NAME).isEqualTo("MySQL");
    }

    @Test
    void shouldUseMySQLVersion() {
        // Uses semantic versioning with caret notation for MySQL 8.x compatibility
        assertThat(MySQLDialectPlugin.DIALECT_VERSION).isEqualTo("^8.0.0");
    }

    @Test
    void shouldDelegateToDifferentRendererThanStandard() {
        SqlDialectPlugin mysqlPlugin = MySQLDialectPlugin.instance();
        SqlDialectPlugin standardPlugin = StandardSQLDialectPlugin.instance();

        SqlRenderer mysqlRenderer = mysqlPlugin.createRenderer();
        SqlRenderer standardRenderer = standardPlugin.createRenderer();

        // Both should be non-null but configured differently
        assertThat(mysqlRenderer).isNotNull();
        assertThat(standardRenderer).isNotNull();
    }
}
