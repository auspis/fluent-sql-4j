...
public class AggregateCallRenderStrategy implements ExpressionRenderStrategy {
    ...
    public String render(AggregateCall expression, SqlRenderer sqlRenderer, AstContext ctx) {
        return switch (expression) {
            case AggregateCallImpl e -> String.format(
                    "%s(%s)", e.operator().name(), e.expression().accept(sqlRenderer, ctx));
            case CountDistinct e -> String.format(
                    "COUNT(DISTINCT %s)", e.expression().accept(sqlRenderer, ctx));
